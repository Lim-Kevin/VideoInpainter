...
class EvaluationService:
    ...
    def __init__(self,
                 ...
                 time_threshold=None,
                 single_object=False,
                 eval_each_object=False):
        if subset not in Davis.sets:
            raise ValueError('Subset must be a valid subset: {}'.format(
                Davis.sets.keys()))

        self.single_object = single_object
        self.eval_each_object = eval_each_object
        self.davis = Davis(davis_root=davis_root, single_object=single_object, eval_each_object=eval_each_object)
    ...
    def summarize_report(self, df):
        ...
        if len(df) == 0:
            ...
            group_columns = ['interaction', 'sequence', 'scribble_idx', 'object_id']
            df = df[group_columns + numeric_columns].groupby(group_columns).mean()
        if 'frame' in df:
