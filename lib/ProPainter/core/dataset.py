...
from lib.ProPainter.inference_propainter import find_bounding_box, crop_images, resize_frames
from lib.ProPainter.utils.file_client import FileClient
from lib.ProPainter.utils.img_util import imfrombytes
from lib.ProPainter.utils.flow_util import resize_flow, flowread
from lib.ProPainter.core.utils import (create_random_shape_with_random_motion, Stack,
...
class TestDataset(torch.utils.data.Dataset):
    def __init__(self, args):
        ...
        self.bounding_box = args['bounding_box']

        # self.video_names = sorted(os.listdir(self.mask_root))
        with open('lib/ProPainter/davis_propainter.json', 'r') as file:
            val_set = json.load(file)
            self.video_names = val_set['val']
    ...
    def __getitem__(self, index):
        ...
        if self.bounding_box:
            full_frames = [np.array(f).astype(np.uint8) for f in frames]

            top_left, bottom_right = find_bounding_box(masks)
            masks, _, crop_size = crop_images(masks, top_left, bottom_right)
            frames, _, _ = crop_images(frames, top_left, bottom_right)
            masks, process_size = resize_frames(masks)
            frames, _ = resize_frames(frames, process_size)
            box_info = {
                'crop_size': crop_size,
                'top_left': top_left,
        ...
        frame_tensors = self._to_tensors(frames) * 2.0 - 1.0
        mask_tensors = self._to_tensors(dilated_masks)

        if self.bounding_box:
            return frame_tensors, mask_tensors, 'None', 'None', video_name, frames_PIL, full_frames, box_info
        else:
            return frame_tensors, mask_tensors, 'None', 'None', video_name, frames_PIL, 'None', 'None'
