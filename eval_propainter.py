...
from lib.ProPainter.model.modules.flow_comp_raft import RAFT_bi
from lib.ProPainter.model.recurrent_flow_completion import RecurrentFlowCompleteNet
from lib.ProPainter.model.propainter import InpaintGenerator

# from core.dataset import TestDataset
from lib.ProPainter.core.dataset import TestDataset
from lib.ProPainter.core.metrics import calc_psnr_and_ssim, calculate_i3d_activations, calculate_vfid, init_i3d_model
...
def main_worker(args):
    ...
    if args.task == 'video_completion':
        eval_summary = open(os.path.join(result_path, "davis_metrics.txt"), "w")
        total_frame_psnr = []
        total_frame_ssim = []
        output_i3d_activations = []
        real_i3d_activations = []
        i3d_model = init_i3d_model('saves/i3d_rgb_imagenet.pt')

    for index, items in enumerate(test_loader):
        torch.cuda.empty_cache()

        # frames, masks, video_name, frames_PIL = items
        frames, masks, flows_f, flows_b, video_name, frames_PIL, full_frames, box_info = items

        if args.bounding_box:
            crop_size = box_info['crop_size']
            top_left = box_info['top_left']
            process_size = box_info['process_size']

            crop_size = [o.item() for o in crop_size]
            top_left = [t.item() for t in top_left]
            process_size = [p.item() for p in process_size]
        ...
        if args.bounding_box:
            # compose frames back together using ori_frames and comp_frames
            ori_frames = [
                full_frames[i].squeeze().cpu().numpy() for i in range(video_length)
            ]

            for i in range(len(comp_frames)):
                crop = comp_frames[i]
                frame = ori_frames[i].copy()
                crop = cv2.resize(crop, crop_size, interpolation=cv2.INTER_LANCZOS4)

                x, y = top_left
                crop_w, crop_h = crop_size
                frame[y:y + crop_h, x:x + crop_w] = crop
                comp_frames[i] = frame
        ...
        if args.task == 'video_completion':
            ...
            for ori, comp in zip(ori_frames, comp_frames):
                ...
                # When there is no mask on the frame, comp and ori are the same and psnr goes to inf
                if math.isinf(psnr):
                    continue
...
if __name__ == '__main__':
    ...
    parser.add_argument('--raft_model_path', default='saves/raft_things.pth', type=str)
    parser.add_argument('--fc_model_path', default='saves/recurrent_flow_completion.pth', type=str)
    parser.add_argument('--propainter_model_path', default='saves/ProPainter.pth', type=str)
    parser.add_argument('--video_root', default='datasets/DAVIS_modified/JPEGImages/480p', type=str)
    parser.add_argument('--mask_root', default='datasets/DAVIS_modified/test_masks', type=str)
